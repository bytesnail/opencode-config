...
文件 `model-capability-heuristics.ts`（打包到 dist/index.js 中）的 `HEURISTIC_MODEL_FAMILY_REGISTRY` 数组中，GLM 族定义如下（自 v4.3.0 起源码路径：`packages/model-core/src/model-capability-heuristics.ts` L75-79；早期版本在仓库根目录同名字段，dist 形态不变）：
...
> - **`dist/tui.js`（TUI 插件）**：opencode 也会通过 `exports["./tui"]` 动态加载此文件，但它**不包含 `resolveCompatibleModelSettings()` 和 `chat.params` hook**（均为 0 处），因此不会 strip `reasoningEffort`。它仅通过 `detectHeuristicModelFamily()` 读取 GLM 注册表用于能力诊断显示（故 TUI 中 GLM 能力显示不包含 `max`——纯显示层面，不影响实际请求参数；本 patch 不覆盖 tui.js）。
...
- 已验证适用版本: 4.15.1, 4.16.0, 4.16.1, 4.16.3, 4.17.1, 4.18.1, 4.18.2
- 最近复核: 2026-07-17 — 4.18.2 源码 `packages/model-core/src/model-capability-heuristics.ts` GLM 族（L75-79）仍缺 `reasoningEfforts`/`max`/`aliases`（PR #5672 仍未合入；`git log --grep=5672` 空；4.18.1→4.18.2 期间 `packages/model-core/src/model-capability-heuristics.ts` 和 `packages/model-core/src/model-settings-compatibility.ts` 均 0 处与 GLM 相关的提交；源码 v4.3.0 起 `refactor(packages): extract model-core package`（commit 2748009ff）将相关文件从仓库根目录移至 `packages/model-core/src/`，dist 形态与 patch 模式不变）；dist `index.js` OLD_CODE 唯一匹配（unpatched L21546-21548）；opencode `packages/opencode/src/provider/transform.ts`（L698-702，对 `@ai-sdk/openai-compatible` 的 GLM-5.2 映射 `max → {reasoningEffort:"max"}`）仍为 GLM-5.2 发出 `reasoningEffort:"max"`；strip 根因 `resolveField` 的 `if (familyKnown)→undefined` 分支（源码 `packages/model-core/src/model-settings-compatibility.ts` L112-113；dist 中 unpatched L21637-21638 / patched L21643-21644）未变。4.18.1 落入主线的 `resolveField` `familyAliases` 形参（源码 L83/L89；dist L21622）以及 `family?.reasoningEffortAliases` 透传调用（dist L21666），与 patch 注入的 `reasoningEffortAliases` 字段天然兼容——别名基础设施已落入主线，仅缺 GLM 条目。dist 于 2026-07-17 00:01 被重新安装（patch 被覆盖），已重新应用并验证（11/11 ✓）。注：4.18.2 仅存在于 opencode 缓存，bun 缓存无对应版本（脚本自动 SKIP bun-cache，不影响运行时——opencode plugin loader 经 `exports["./server"]`（首选，`packages/opencode/src/plugin/install.ts` L149）或 `pkg.main`（回退，L152）只加载 `dist/index.js`，tui 走独立的 `exports["./tui"]`→`dist/tui.js` 路径）；`dist/tui.js` 有独立的未 patch GLM 注册表副本，仅用于诊断显示，不影响请求参数（`resolveCompatibleModelSettings`/`chat.params` hook 在 tui.js 中均为 0 处）
- 脚本自动适配当前版本（从 opencode.jsonc 提取），无需手动修改版本号
- 备份文件名含版本号（如 `index.js.opencode-cache.4.15.1.original`、`index.js.opencode-cache.4.16.0.original`、`index.js.opencode-cache.4.16.1.original`、`index.js.opencode-cache.4.16.3.original`、`index.js.opencode-cache.4.17.1.original`、`index.js.opencode-cache.4.18.1.original`、`index.js.opencode-cache.4.18.2.original`），不同版本的备份互不覆盖
- PR #5672 截至 2026-07-17 仍未合入（源码 L75-79 仍缺字段，git log 无相关 fix 提交）；合入后本 patch 可移除（脚本 `--check` 会自动检测官方修复并跳过）
