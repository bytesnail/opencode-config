...
- 已验证适用版本: 4.15.1, 4.16.0, 4.16.1, 4.16.3, 4.17.1, 4.18.1
- 最近复核: 2026-07-14 — 4.18.1 源码 `model-capability-heuristics.ts` GLM 族（L76-79）仍缺 `reasoningEfforts`/`max`/`aliases`（PR #5672 仍未合入；4.17.1→4.18.1 期间 `model-capability-heuristics.ts` 和 `model-settings-compatibility.ts` 均 0 处提交，patch 模式完全不变）；dist `index.js` OLD_CODE 唯一匹配（unpatched L21505-21508）；opencode `src/provider/transform.ts`（L698-703，对 `@ai-sdk/openai-compatible` 的 GLM-5.2 映射 `max → {reasoningEffort:"max"}`）仍为 GLM-5.2 发出 `reasoningEffort:"max"`；strip 根因 `resolveField` 的 `if (familyKnown)→undefined` 分支（源码 `model-settings-compatibility.ts` L112-113；dist 中 unpatched L21596-21598 / patched L21602-21604）未变。4.18.1 新增 `resolveField` 的 `familyAliases` 形参（dist L21581）以及 `family?.reasoningEffortAliases` 透传调用（dist L21625），与 patch 注入的 `reasoningEffortAliases` 字段天然兼容——别名基础设施已落入主线，仅缺 GLM 条目。dist 于 2026-07-14 23:32 被重新安装（patch 被覆盖），已重新应用并验证（11/11 ✓）。注：4.18.1 仅存在于 opencode 缓存，bun 缓存无对应版本（脚本自动 SKIP bun-cache，不影响运行时——opencode plugin loader 经 `exports["./server"]`（首选，`packages/opencode/src/plugin/install.ts` L149）或 `pkg.main`（回退，L152）只加载 `dist/index.js`，tui 走独立的 `exports["./tui"]`→`dist/tui.js` 路径）；`dist/tui.js` 有独立的未 patch GLM 注册表副本，仅用于诊断显示，不影响请求参数（`resolveCompatibleModelSettings`/`chat.params` hook 在 tui.js 中均为 0 处）
- 脚本自动适配当前版本（从 opencode.jsonc 提取），无需手动修改版本号
- 备份文件名含版本号（如 `index.js.opencode-cache.4.15.1.original`、`index.js.opencode-cache.4.16.0.original`、`index.js.opencode-cache.4.16.1.original`、`index.js.opencode-cache.4.16.3.original`、`index.js.opencode-cache.4.17.1.original`、`index.js.opencode-cache.4.18.1.original`），不同版本的备份互不覆盖
- PR #5672 截至 2026-07-14 仍未合入（源码 L75-79 仍缺字段，git log 无相关 fix 提交）；合入后本 patch 可移除（脚本 `--check` 会自动检测官方修复并跳过）
